...
const CompanieDetails = () =>  {
  ...
  useEffect(() => {
    if (!isFetch && companie.length === 0) getAllCompanies();
  }, [isFetch, getAllCompanies, companie.length]);

  useEffect(() => {
    if (company?.location) {
      getCoordinatesByAddress(company.location).then(setCoordinates);
    }
  }, [company]);
    ...
    <>
      ...
      <div className="flex flex-wrap ml-[30px] mt-[20px]">
        ...
        <div className="flex flex-wrap items-center gap-[20px] ml-[20px]">
          <p className="text-[20px]">{company.name}</p>
      ...
      <div className="w-[60%] ml-[30px] mt-[30px]">
        ...
        <ul className="list-disc pl-5">
          {company.responsibilities &&
            Object.values(company.responsibilities).map(
